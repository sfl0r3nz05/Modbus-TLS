...
# --------------------------------------------------------------------------- #
class ModbusTLSClient(BaseModbusClient):
    ...
    def __init__(self, host='127.0.0.1', port=Defaults.Port,
        ...
        logger.debug("using caCertFile={0}, deviceCertFile={1}, deviceKeyFile={2}".format(caCertFile, deviceCertFile, deviceKeyFile))
        sslSocket = ssl.wrap_socket(sock, ca_certs=caCertFile, cert_reqs=ssl.CERT_REQUIRED, certfile=deviceCertFile, keyfile=deviceKeyFile, ssl_version=ssl.PROTOCOL_TLSv1_2, do_handshake_on_connect=True, ciphers='ECDHE-RSA-AES256-SHA')
    ...
    def verify_cert(self):
        ...
        """
        if self.socket:
            servercert = self.socket.getpeercert()
            subject = dict(x[0] for x in servercert['subject'])
            logger.info("Connected to server with valid certificate, CN={0}".format(subject['commonName']))  
